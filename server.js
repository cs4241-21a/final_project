    ...
    user.model.countDocuments(
        ...
        function (err, count) {
            ...
            if (count > 0) {
                ...
                res.json({ failed: "false", username: req.session.user });
            } else {
...
app.post("/changePassword", (req, res) => {
    const filter = { un: req.session.user }
    const update = { pw: req.body.newPassword }

    user.model.findOneAndUpdate(filter, update, { new: true },
        function (err, doc) {
            if (!err) {
                console.log("Success with user {" + req.session.user + "}!");
                res.json({ failed: "false", username: req.session.user });
            } else {
                res.json({ failed: "noUser" });
