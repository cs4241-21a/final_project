...
In order to utilize this web application, users must have a preexisting account registered with Microsoft Outlook, Google, GitHub, and/or Discord. Users should note that if they were to login with Outlook, logout, then login again with GitHub, these would be treated as separately registered accounts and the profile and post information will not transfer over. In order to select multiple options on the profile or the dashboard page for skills/languages/courses, press CTRL/CMD depending on your machine and then click on the desired options.
